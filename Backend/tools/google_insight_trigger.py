"""
Tool for triggering Google Ads Insight workflows via the FastAPI backend.
"""
...
async def google_insight_trigger(tool_input: str) -> str:
    """
    Triggers a Google Ads Insight workflow in the backend.
    ...
    try:
        ...
        endpoint = f"{backend_url}/api/v1/workflows/trigger/google_insight"
        ...
        async with httpx.AsyncClient() as client:
            ...
            if response.status_code == 200:
                ...
                return f"Google Ads insight analysis started (workflow ID: {workflow_id}). Results will appear in the dashboard shortly."
            else:
...
google_insight_trigger_tool = Tool(
    name="google_insight_trigger",
    description="""
    Useful for answering questions about Google Ads performance metrics and insights.
    Use this tool when the user asks about Google ad performance, CTR, CPC, impressions, clicks,
    campaign or ad group performance, conversion rates, or other Google advertising metrics.
    The analysis will be displayed in the dashboard.
    The input must be a JSON string with "query" and "session_id" fields.
    Example input: {"query": "how is my Google campaign performing", "session_id": "user-123456"}
    """,
    func=lambda x: None,  # Placeholder
    coroutine=google_insight_trigger,
